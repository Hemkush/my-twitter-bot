...
class BotScheduler:
    ...
    def add_interval_schedule(self, hours=6):
        """Add an interval-based scheduled task"""
        self.scheduler.add_job(
            func=self.run_bot_task,
            trigger=IntervalTrigger(hours=hours),
            id='interval_post',
            name=f'Post every {hours} hours',
            replace_existing=True
        )
        logger.info(f"Added interval schedule every {hours} hours")
...
def main():
    ...
    # bot_scheduler.add_interval_schedule(hours=6)
